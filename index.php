...
if(isset($_REQUEST['spinrewriter_api']) && isset($_REQUEST['spinrewriter_api']))
{
	$api = $_REQUEST['spinrewriter_api'];
	update_option('spinrewriter_api',$api);
}
if(isset($_REQUEST['spinrewriter_email']) && isset($_REQUEST['spinrewriter_email']))
{
...
class spinRewriter extends SanityPluginFramework {
    ...
    $temp_api   = get_option('spinrewriter_api',false);
     if($temp_email == false)
      ...
      $this->data['email'] = trim(get_option('spinrewriter_email'));
      $this->data['api'] = trim(get_option('spinrewriter_api'));
if(empty($this->data['email'] ) || $this->data['email'] == 'not set' || $this->data['api'] == 'not set' || empty($this->data['api']))
{
...
?>
<div class="error">
<p>SpinRewriter email and/or api key is missing. <b>Goto settings->spin rewriter options page</b>
Click <a href="http://www.spinrewriter.com/?ref=d4d7">here</a> to get an api key.
</p>
...
<?php
	...
	function add_options_page () {
		add_options_page('SpinRewriter','SpinRewriter','manage_options','spin_rewriter',array($this, 'render_page'));
	}
	...
	{
	add_meta_box( 'Spin-Rewriter', 'SpinRewriter', array(&$this,'render_meta_box'), 'post', 'side', 'high' );  
		add_meta_box( 'Spin-Rewriter', 'SpinRewriter ', array(&$this,'render_meta_box'), 'page', 'side', 'high' );  
		
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    		add_meta_box( 'Spin-Rewriter', 'SpinRewriter ', array(&$this,'render_meta_box'), 'product', 'side', 'high' );  
